...
app.get('/iphones', async (req, res) => {
    // Список моделей iPhone для запроса
    const models = ['iPhone 15', 'iPhone 15 Pro', 'iPhone 15 Pro Max'];
    const devicesInfo = await Promise.all(models.map(model => getDeviceInfo(model)));
    
    // Отфильтровываем null значения в случае ошибок
    const validDevices = devicesInfo.filter(device => device !== null);

    res.render('iphones', { devices: validDevices });
});
...
// Это примерный код, показывающий, как могли бы быть организованы данные и их передача в EJS.
    ...
    const devicesInfo = [
        {
            ...
            primary_camera: '48 MP, f/1.6, 26mm (wide), dual pixel PDAF, OIS'
        },
        {
            DeviceName: 'iPhone 15 Pro',
            Brand: 'Apple',
            technology: 'Super Retina XDR display with ProMotion',
            primary_camera: 'Triple 48 MP, f/1.5, 26mm (wide), dual pixel PDAF, OIS'
        },
        {
            DeviceName: 'iPhone 15 Pro Max',
            Brand: 'Apple',
            technology: 'Super Retina XDR display with ProMotion and HDR10',
            primary_camera: 'Triple 48 MP, f/1.5, 26mm (wide), dual pixel PDAF, OIS'
        }
    ];
